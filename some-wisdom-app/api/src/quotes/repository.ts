...
export class FileQuoteNoteRepository implements QuoteNoteRepository {
  ...
  async update(note: QuoteNote): Promise<QuoteNote> {
    const quoteNotes = await this.allQuoteNotesFromDb();

    const quoteNotesWithoutUpdated = quoteNotes.filter(q => q.noteId != note.noteId);

    const updatedQuoteNotes = [...quoteNotesWithoutUpdated, note];

    await Files.writeTextFileContent(this.dbFilePath, JSON.stringify(updatedQuoteNotes));
